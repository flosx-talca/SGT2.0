...
with app.app_context():
    # Buscar ausencias en Mayo 2026
    inicio_mayo = datetime(2026, 5, 1).date()
    fin_mayo = datetime(2026, 5, 31).date()
    
    ausencias = TrabajadorAusencia.query.filter(
        TrabajadorAusencia.fecha_inicio <= fin_mayo,
        TrabajadorAusencia.fecha_fin >= inicio_mayo
    ).all()
    
    print(f"Ausencias en Mayo 2026 ({len(ausencias)}):")
    for a in ausencias:
